import { DataParser } from "./test.mjs"
...
//     profit=Math.round(data[i][2]*100)/100;
//     totprofit+=profit;
// data[i][2]=profit;
//     sales = Math.round(data[i][3]*100)/100;
//     totsales+=sales;
//     data[i][3]=sales;
//   }
...
//   data[0].push("sales+10%") ;
//   data[0].push("profit + 5%");
//   for(let i = 1 ; i < data.length ; i++ ){
//       data[i].push(data[i][3]+((10 / 100)*data[i][3]));
//       if(data[i][0] == 'Technology'){
//           data[i].push(data[i][2]+((5 / 100)*data[i][2]));
//       }  
//   }
//   for(let i = 0 ; i < data.length ; i++ ){
//       console.log(data[i])
...
const dataParser = new DataParser(data);
  dataParser.roundOffColumn('Profit',5);
  dataParser.roundOffColumn('Sales',4);
  dataParser.logData();
