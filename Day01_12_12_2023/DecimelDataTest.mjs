import { Decimal } from './Decimel.mjs';
...
const data = [
  ...
  const dataParser = new Decimal(data);
  dataParser.roundOffColumn('Profit', 2);
  dataParser.logData();
  dataParser.filterColumn('Category', 'Technology');
  ...
  const result = dataParser.filterRow(filterList);
  console.log(result);
