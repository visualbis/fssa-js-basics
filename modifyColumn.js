  ...
  const modifyColumnPercentage = (data, columnName, percentage) => {
    return data.map((row, index) => {
      ...
      const modifiedRow = row.map((value, columnIndex) => {
        if (columnIndex === data[0].indexOf(columnName)) {
          const modifiedValue = Math.round(parseFloat(value) * (1 + percentage / 100));
  ...
  const modifiedData = modifyColumnPercentage(data, "Profit", 5);
